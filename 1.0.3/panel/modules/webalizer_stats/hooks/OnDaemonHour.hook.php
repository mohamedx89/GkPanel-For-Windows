...
/**
 *
 * XPanel - Visitor Stats XPanel plugin, written by HuyGK: www.cybercore.tv.
 *
 */
...
function GenerateWebalizerStats()
{
    ...
    while ( $rowvhost = $sql->fetch() ) {
        if ( !file_exists( ctrl_options::GetSystemOption( 'XPanel_root' ) . "modules/webalizer_stats/stats/" . $rowvhost[ 'ac_user_vc' ] . "/" . $rowvhost[ 'vh_name_vc' ] ) ) {
            @mkdir( ctrl_options::GetSystemOption( 'XPanel_root' ) . "modules/webalizer_stats/stats/" . $rowvhost[ 'ac_user_vc' ] . "/" . $rowvhost[ 'vh_name_vc' ], 0755, TRUE );
        }

        /** set webalizer command dependant on OS */
        if ( sys_versions::ShowOSPlatformVersion() == "Windows" ) {
            $command = ctrl_options::GetSystemOption( 'XPanel_root' ) .
                'modules/webalizer_stats/bin/webalizer.exe';
        }
        else {
            chmod( ctrl_options::GetSystemOption( 'XPanel_root' ) . "modules/webalizer_stats/bin/webalizer", 4777 );
            $command = "webalizer";
        }
        ...
        $statsPath = ctrl_options::GetSystemOption( 'XPanel_root' ) .
