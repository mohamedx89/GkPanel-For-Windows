...
while ($modules = $modsql->fetch()) {
    ...
    $mod_config = new xml_reader(fs_filehandler::ReadFileContents(ctrl_options::GetSystemOption('gkpanel_root') . 'modules/' . $modules['mo_folder_vc'] . '/module.xml'));
    $mod_config->Parse();
...
/*
 * Please DO NOT remove the below code, this helps us at the GKPanel project
 * find out non-personal infomation about how people are running GKPanel. The only infomation
 * that we are passing back here is just your GKPanel version and what OS you are running it on.
 *
