 ...
 * @copyright (c) 2013 XPanel Group - http://www.cybercore.tv/
 * @license http://opensource.org/licenses/gpl-3.0.html GNU Public License v3
 */

// The Main GKPanel.js file requires these libraries to assist it and make it all work...
// /js/jquery.js
// /js/jquery.cookie.js
...
var GKPanel = {
    ...
    init: function() {
        ...
        GKPanel.utils.log('GKPanel.init() ran');
        GKPanel.menu.header();
        GKPanel.menu.sidebar();
        GKPanel.loader.init();
        GKPanel.stats.init();
        GKPanel.modules.dragDrop();
        GKPanel.modules.boxes();
        ...
        //GKPanelDNS.utils.cache.dnsTitleId = $("#dnsTitle");
    ...
    utils: {
        ...
        log: function(what) {
            if (GKPanel.utils.settings.debug) {
                console.log(what);
            }
        },
        ...
        deepExtend: function(destination, source) {
            ...
            for (property in source) {
                if (source[property] && source[property].constructor && source[property].constructor === Object) {
                    //GKPanel.utils.log(source[property].constructor);
                    destination[property] = destination[property] || {};
                    GKPanel.utils.deepExtend(destination[property], source[property]);
                } else if (property in destination) {
                    destination[property] = source[property];
                } else {
                    throw new Error('GKPanel.deepExtend was passed a non-supported Property: ' + property);
                }
            }
            return destination;
        },
    ...
    loader: {
        ...
        init: function() {
            GKPanel.utils.log('GKPanel.loader.init() ran - Watching for Click events');
            //Bind zloader to button click
            $('#button').click(function() {
                GKPanel.loader.showLoader();
            });
        ...
        showLoader: function() {
            ...
            GKPanel.utils.log('GKPanel.loader.showLoader() ran - Show Spinning Loader screen');
            $('#zloader_overlay').fadeIn('fast', function() {
                $("#zloader").show();
                GKPanel.loader.buildSpinner();
            });
        },

        hideLoader: function() {
            //Hide  Spinning Loader
            GKPanel.utils.log('GKPanel.loader.hideLoader() ran - Remove Spinning Loader screen');
            $('#zloader_overlay').fadeOut('fast', function() {
                $("#zloader").hide();
                GKPanel.loader.spinner.stop();
            });
        },
        ...
        buildSpinner: function() {
            GKPanel.utils.log('GKPanel.loader.buildSpinner() ran - Builing Spining Loader');
            var opts = {
            ...
            GKPanel.loader.spinner = new Spinner(opts).spin(target);
        },
    },
    ...
    notice: {
        ...
        show: function(options) {
            ...
            GKPanel.utils.log('GKPanel.notice.show() ran - Show Notice');
            // Default options
            this.options = {
            ...
            if (this.options.autoClose) {
                setTimeout(function() {
                    GKPanel.notice.hide(selector);
                }, this.options.closeTime);
            }
        },

        hide: function(selector) {
            GKPanel.utils.log('GKPanel.notice.hide() ran - Hide Notice');
    ...
    dialog: {
        ...
        confirm: function(options) {
            ...
            GKPanel.utils.log('GKPanel.dialog.confirm() ran - Show Dialog');
            // Default options
            this.options = {
                title: 'ATTENTION',
                message: '',
            ...
            GKPanel.utils.deepExtend(this.options, options);
            ...
            if (this.options.okButton.show !== false) {
                ...
                GKPanel.utils.addEvent(ok, 'click', function() {
            ...
            if (this.options.cancelButton.show !== false) {
                ...
                GKPanel.utils.addEvent(cancel, 'click', function() {
    ...
    stats: {
        init: function() {
            ...
            $('#stats-tab a').click(function(e) {
                GKPanel.utils.log('GKPanel.stats.init Click Event - Toggle Stats Tabs');
                e.preventDefault();
                $(this).tab('show');
            })
        }
    ...
    menu: {
        ...
        sidebar: function() {
            ...
            GKPanel.utils.log('GKPanel.menu.sidebar() Ran - Handle sidebar menu clicks');
    ...
    modules: {
        ...
        dragDrop: function() {
            GKPanel.utils.log('GKPanel.modules.dragDrop() Ran - Handle Module Box Sorting');
            $('.sortable').sortable({
                handle: '.handle',
                onStartDrag: function() {
                    GKPanel.modules.addFloats($(".sortable"));
                },
                onEndDrag: function() {
                    GKPanel.modules.addFloats($(".sortable"))
                },
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        GKPanel.utils.log('GKPanel.modules.dragDrop() AJAX Sorting order Saved');
                        GKPanel.loader.hideLoader();
                    },

                    error: function(ts) {
                        GKPanel.utils.log('ERROR: GKPanel.modules.dragDrop() AJAX Sorting order NOT Saved');
                        GKPanel.loader.hideLoader();
                    }
        ...
        addFloats: function(container) {
            GKPanel.utils.log('GKPanel.modules.addFloats() - Floats added for Drag/Drop Modules');
        ...
        boxes: function() {
            ...
            $('.module-box-title .tools .expand, .module-box-title .tools .collapse').click(function(e) {
                GKPanel.utils.log('modulebox clicked');
        ...
        typeAhead: function(moduleNames) {
            ...
            $('#module-search').typeahead({
                ...
                var $typeahead = $(this);
                GKPanel.utils.log(moduleUrls[datum.value]);
...
$(function() {
    GKPanel.utils.settings.debug = false;
    GKPanel.init();
});
